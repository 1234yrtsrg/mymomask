...
class MaskTransformerTrainer:
    ...
    def save(self, file_name, ep, total_it):
        if not self.is_main:
            return
        t2m_trans_state_dict = unwrap_model(self.t2m_transformer).state_dict()
    ...
    def resume(self, model_dir):
        ...
        missing_keys, unexpected_keys = unwrap_model(self.t2m_transformer).load_state_dict(checkpoint['t2m_transformer'], strict=False)
    ...
    def train(self, train_loader, val_loader, *unused_args, **unused_kwargs):
        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch in enumerate(train_loader):
                ...
                logs['loss'] += reduce_mean(loss, self.device)
                logs['acc'] += reduce_mean(acc, self.device)
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    loss, acc = self.forward(batch_data)
                    val_loss.append(reduce_mean(loss.item(), self.device))
                    val_acc.append(reduce_mean(acc, self.device))
            ...
            if self.logger is not None:
                ...
                if np.mean(val_loss) < best_loss:
                    print(f"Improved validation loss from {best_loss:.02f} to {np.mean(val_loss):.3f}")
                    self.save(pjoin(self.opt.model_dir, 'net_best_loss.tar'), epoch, it)
                    best_loss = np.mean(val_loss)

                if np.mean(val_acc) > best_acc:
                    print(f"Improved accuracy from {best_acc:.02f} to {np.mean(val_acc):.3f}")
...
class ResidualTransformerTrainer:
    ...
    def save(self, file_name, ep, total_it):
        if not self.is_main:
            return
        res_trans_state_dict = unwrap_model(self.res_transformer).state_dict()
    ...
    def resume(self, model_dir):
        checkpoint = torch.load(model_dir, map_location=self.device)
        missing_keys, unexpected_keys = unwrap_model(self.res_transformer).load_state_dict(checkpoint['res_transformer'], strict=False)
        assert len(unexpected_keys) == 0
        assert all([k.startswith('clip_model.') for k in missing_keys])
